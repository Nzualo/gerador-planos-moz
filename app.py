...
def create_pdf(inputs, dados, obj_geral, obj_especificos):
    ...
    # OBJETIVOS SEM REPETIÇÃO DO CABEÇALHO
...
def gerar_plano(instrucoes_arquivo="", instrucoes_ajuste="", arquivo=None):
    ...
    try:
        status_text.text("Conectando ao modelo superior...")
        progress_bar.progress(10)
        genai.configure(api_key=st.secrets["GOOGLE_API_KEY"])
        
        # Uso do modelo Pro para maior precisão nas tabelas
        model = genai.GenerativeModel('gemini-1.5-pro')
        
        prompt = f"""Aja como Pedagogo do SNE Moçambique. Gere um plano de aula técnico.
        ...
        REGRAS CRÍTICAS:
        1. NÃO inclua informações de identificação (Escola, Professor, Disciplina) nos blocos de texto. O professor preencherá isso à mão.
        2. FOQUE apenas no Objetivo Geral, Objetivos Específicos e na Tabela.
        3. A tabela DEVE ter 6 colunas rigorosamente separadas por ||.
        
        FORMATO DE SAÍDA:
        [BLOCO_GERAL] (Apenas a frase do objetivo geral) [FIM_GERAL]
        [BLOCO_ESPECIFICOS] (Lista de objetivos específicos) [FIM_ESPECIFICOS]
        [BLOCO_TABELA] (Linhas da tabela com ||) [FIM_TABELA]
        """
        ...
        status_text.text("Analisando e construindo tabela alinhada...")
...
comando_ia_arquivo = st.text_input("🤖 Comando para a IA sobre o ficheiro", placeholder="Ex: Use apenas o texto da pág 10...")
...
if st.session_state.get('plano_pronto'):
    ...
    st.download_button(label="📄 Baixar PDF Final", data=pdf_bytes, file_name="Plano_Aula_Final.pdf", mime="application/pdf", type="primary", use_container_width=True)
